...
const items = [
  { href: '/app2/dashboard', label: 'Dashboard' },
]

function usePath() {
  const [path, setPath] = React.useState<string>(typeof window !== 'undefined' ? getPath() : '/app2/dashboard')
  React.useEffect(() => {
  ...
  return (
    <div className={cn('sticky top-0 z-30 w-full border-b bg-background/80 backdrop-blur', className)}>
      <div className="px-2 md:px-0 py-1">
        <h2 className="text-sm font-semibold text-muted-foreground">App 2</h2>
      </div>
